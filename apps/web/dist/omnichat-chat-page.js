...
${c}`:c,s=s?`${s}
${f}`:f;let d=this.lexer.state.top;if(this.lexer.state.top=!0,this.lexer.blockTokens(f,i,!0),this.lexer.state.top=d,n.length===0)break;let y=i.at(-1);if(y?.type==="code")break;if(y?.type==="blockquote"){let k=y,I=k.raw+`
`+n.join(`
`),N=this.blockquote(I);i[i.length-1]=N,r=r.substring(0,r.length-k.raw.length)+N.raw,s=s.substring(0,s.length-k.text.length)+N.text;break}else if(y?.type==="list"){let k=y,I=k.raw+`
`+n.join(`
`),N=this.list(I);i[i.length-1]=N,r=r.substring(0,r.length-y.raw.length)+N.raw,s=s.substring(0,s.length-k.raw.length)+N.raw,n=I.substring(i.at(-1).raw.length).split(`
...
`,1)[0],k=!d.trim(),I=0;if(this.options.pedantic?(I=2,f=d.trimStart()):k?I=e[1].length+1:(I=d.search(this.rules.other.nonSpaceChar),I=I>4?1:I,f=d.slice(I),I+=e[1].length),k&&this.rules.other.blankLine.test(y)&&(c+=y+`
...
`+ke.slice(I);else{if(k||d.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||X.test(d)||H.test(d)||le.test(d))break;f+=`
`+y}k=!y.trim(),c+=ee+`
