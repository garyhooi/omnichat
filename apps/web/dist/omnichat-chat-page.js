...
`,t=t.substring(b.length+1),l=!0),!l){let P=this.rules.other.nextBulletRegex(I),ce=this.rules.other.hrRegex(I),Z=this.rules.other.fencesBeginRegex(I),H=this.rules.other.headingBeginRegex(I),X=this.rules.other.htmlBeginRegex(I),O=this.rules.other.blockquoteBeginRegex(I);for(;t;){let ee=t.split(`
`,1)[0],ve;if(b=ee,this.options.pedantic?(b=b.replace(this.rules.other.listReplaceNesting,"  "),ve=b):ve=b.replace(this.rules.other.tabCharGlobal,"    "),Z.test(b)||H.test(b)||X.test(b)||O.test(b)||P.test(b)||ce.test(b))break;if(ve.search(this.rules.other.nonSpaceChar)>=I||!b.trim())f+=`
